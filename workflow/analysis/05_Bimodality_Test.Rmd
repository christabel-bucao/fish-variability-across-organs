...
zscore.matrix <- lapply(setNames(conditions, conditions), function(ts) {
  compute_zscore(filtered.samples.retained$log2.tmm.cpm.gf[[ts]])
})
```
...
for (ts in conditions) {
  ...
  bimodality.test[[ts]]$VarRankBin <- ceiling(bimodality.test[[ts]]$VarRank*10)/10
  bimodality.test[[ts]]$VarRankBin[round(bimodality.test[[ts]]$VarRankBin,1)==0.0] <- 0.1
}
```
