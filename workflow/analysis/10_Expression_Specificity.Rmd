...
if (!params$qc1.only) {
  if (!params$all.nonzero.matrix) {
    filtered.samples$LOC <- readRDS(
      file.path(input.path, "LOC_2_reps_data.qc2.cor.rds"))
    
    filtered.samples$ELU <- readRDS(
      file.path(input.path, "ELU_1_reps_data.qc2.cor.rds"))
    
    filtered.samples$DRE <- readRDS(
      file.path(input.path, "DRE_1_reps_data.qc2.cor.rds"))    

} else {
    filtered.samples$LOC <- readRDS(
      file.path(input.path, "LOC_2_reps_data.qc2.cor.nonzero.rds"))
    
    filtered.samples$ELU <- readRDS(
      file.path(input.path, "ELU_1_reps_data.qc2.cor.nonzero.rds"))
  
    filtered.samples$DRE <- readRDS(
      file.path(input.path, "DRE_1_reps_data.qc2.cor.nonzero.rds"))    

  }  
} else {
    filtered.samples$LOC <- readRDS(
      file.path(input.path, "LOC_2_reps_data.qc1.seq.rds"))
    
    filtered.samples$ELU <- readRDS(
      file.path(input.path, "ELU_1_reps_data.qc1.seq.rds"))
    
    filtered.samples$DRE <- readRDS(
      file.path(input.path, "DRE_1_reps_data.qc1.seq.rds"))      
}
